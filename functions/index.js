...
// (Your Teams List - Keep this exactly as it matches your Frontend)
const RAW_TEAMS = {
...
const getTargetDates = () => {
    ...
    // Helper to format Date object to YYYYMMDD in New York Time
...
// --- 3. REGISTER TOKEN FUNCTION (HTTP) ---
...
// --- 4. CHECK SCORES FUNCTION (SCHEDULED) ---
exports.checkScores = onSchedule("every 5 minutes", async (event) => {
    const datesToCheck = getTargetDates();
    console.log(`🔍 Checking scores for dates: ${datesToCheck.join(', ')} (Eastern Time)`);

    // Force unique request to bypass caching
    const ts = Date.now();
    ...
    for (const league of leagues) {
        ...
        datesToCheck.forEach(dateStr => {
            // Updated: Added limit=1000 and cache-buster
            urls.push(`https://site.api.espn.com/apis/site/v2/sports/${sportPath}/scoreboard?dates=${dateStr}&limit=1000&_=${ts}`);
            if (league === 'NBA' || league === 'CFB') {
                urls.push(`https://site.api.espn.com/apis/site/v2/sports/${sportPath}/scoreboard?dates=${dateStr}&seasontype=2&limit=1000&_=${ts}`);
                urls.push(`https://site.api.espn.com/apis/site/v2/sports/${sportPath}/scoreboard?dates=${dateStr}&seasontype=3&limit=1000&_=${ts}`);
            }
        });
        ...
        try {
            ...
            for (const event of uniqueEvents) {
                // Ensure we are only looking at games from our Target NY Dates
                const gameDateNY = new Date(event.date).toLocaleDateString("en-CA", { timeZone: "America/New_York" }).replace(/-/g, '');
                if (!datesToCheck.includes(gameDateNY)) continue;

                if (!event.status.type.completed) continue;

                // CHANGED: Extended window to 24 hours to catch early Saturday games processed late
                ...
                if (loserTeamId) {
                    ...
                    try {
                        await db.runTransaction(async (t) => {
                            ...
                            const isBasketball = league === 'NBA' || league === 'NCAA';
                            const isBigMargin = isBasketball && margin > 10;
                            
                            let messageStyles;
                            
                            if (isBigMargin) {
                              messageStyles = [
                                `All is right with the world. ${teamName} lost by ${margin}! 🌍`,
                                `${teamName} got demolished by ${margin} points. 📉`,
                                `That's what I'm talking about. ${teamName} lost by ${margin}! 👏`,
                                `Poetry. ${teamName} down by ${margin}. 📜`,
                                `Chef's kiss. ${teamName} lost by ${margin} points. 👨‍🍳💋`,
                                `My day just got better. ${teamName} lost by ${margin}. ☀️`
                              ];
                            } else {
                              messageStyles = [
                                `So... ${winnerName} beat ${teamName}. 😬`,
                                `Yes! ${winnerName} took down ${teamName}.`,
                                `${teamName} lost to ${winnerName}. 👀`,
                                `Couldn't happen to a nicer team. ${teamName} lost to ${winnerName}. 🙃`,
                                `${teamName} just made my day. Lost to ${winnerName}. 🎉`,
                                `${teamName} took the L against ${winnerName}. 😏`,
                                `That's what I'm talking about. ${winnerName} beat ${teamName}. 👏`,
                                `${teamName} adding to the highlight reel... of losses. Lost to ${winnerName}. 📹`,
                                `The universe is in balance. ${teamName} lost to ${winnerName}. ⚖️`,
                                `FINAL: ${winnerName} beats ${teamName}.`,
                                `THEY LOST! ${teamName} fell to ${winnerName}.`,
                                `GAME OVER: ${teamName} loses to ${winnerName}.`,
                                `Christmas came early. ${teamName} lost to ${winnerName}. 🎄`
                              ];
                            }
                            
                            const randomMessage = messageStyles[Math.floor(Math.random() * messageStyles.length)];
                            ...
                            usersSnapshot.forEach(userDoc => {
                                ...
                                notifications.push(
                                    fcm.send(payload).catch(err => console.error(err))
                                );
                            });
                        ...
                        console.error(`❌ Transaction failed for game ${event.id}:`, error);
                    }
                }
            }
        } catch (err) {
